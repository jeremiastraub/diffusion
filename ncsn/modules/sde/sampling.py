...
from .sde import VESDE, VPSDE, subVPSDE, SDE
...
def get_sampling_fn(
    ...
    *,
    ...
    Args:
        sampling_method:
        sde: A `sde.SDE` object that represents the forward SDE.
        shape: A sequence of integers representing the expected shape of a
            single sample (B x C x H x W).
        class_conditional: Whether the score model was conditioned on class
            labels [0, ..., model.num_classes].
        ...
        snr:
        n_steps_each:
        class_idx: Specifies the class to sample from if class_conditional is
            True. If None, choose a class index from a uniform distribution.
        probability_flow:
        continuous:
        as_generator:
        ode_rtol:
    ...
    Returns: A function that takes random states and a replicated training
        ...
        `shape`.
    """
    assert len(shape) == 4, "'shape' must be of the form (B, C, H, W)"

    # Probability flow ODE sampling with black-box ODE solvers
    if sampling_method.lower() == 'ode':
    ...
    # are special cases.
    elif sampling_method.lower() == 'pc':
        pred = get_predictor(predictor.lower())
        corr = get_corrector(corrector.lower())
        ...
        raise ValueError(
            f"Sampling method {sampling_method} unknown. Must be one of: "
            "ode, pc"
        )
...
class Predictor(abc.ABC):
    ...
    def update_fn(self, x, t, y=None):
        ...
        Args:
            ...
            y: A PyTorch tensor representing the (class) conditioning.
...
class EulerMaruyamaPredictor(Predictor):
    ...
    def update_fn(self, x, t, y=None):
        ...
        drift, diffusion = self.rsde.sde(x, t, y=y)
        x_mean = x + drift * dt
...
class ReverseDiffusionPredictor(Predictor):
    ...
    def update_fn(self, x, t, y=None):
        f, G = self.rsde.discretize(x, t, y=y)
        z = torch.randn_like(x)
        x_mean = x - f
...
class AncestralSamplingPredictor(Predictor):
    ...
    def vpsde_update_fn(self, x, t, y=None):
        sde = self.sde
        timestep = (t * (sde.N - 1) / sde.T).long()
    ...
    def update_fn(self, x, t, y=None):
        if isinstance(self.sde, VESDE):
            return self.vesde_update_fn(x, t, y=y)
        elif isinstance(self.sde, VPSDE):
            return self.vpsde_update_fn(x, t, y=y)
...
def shared_predictor_update_fn(
    x, t, *, sde, model, predictor, probability_flow, continuous, y=None
):
    ...
    return predictor_obj.update_fn(x, t, y=y)


def shared_corrector_update_fn(
    x, t, *, sde, model, corrector, continuous, snr, n_steps, y=None
):
    ...
    return corrector_obj.update_fn(x, t, y=y)

# TODO Instead of device kwarg, always use the model's device
...
def get_pc_sampler(
    ...
    Args:
        sde: An `sde.SDE` object representing the forward SDE.
            ...
            with torch.no_grad():
                ...
                if class_conditional:
                    if class_idx is not None:
                        assert class_idx < model.num_classes
                        y = torch.tensor(shape[0]*[class_idx], device=device)
                    else:
                        y = torch.randint(
                            model.num_classes, size=(shape[0],), device=device
                        )
                else:
                    y = None
                ...
                for i in range(sde.N):
                    ...
                    x, x_mean = corrector_update_fn(x, vec_t, y=y, model=model)
                    x, x_mean = predictor_update_fn(x, vec_t, y=y, model=model)
...
def get_ode_sampler(
    ...
    def denoise_update_fn(model, x, y=None):
        score_fn = get_score_fn(sde, model, continuous=True)
        # Reverse diffusion predictor for denoising
        predictor_obj = ReverseDiffusionPredictor(sde, score_fn, probability_flow=False)
        vec_eps = torch.ones(x.shape[0], device=x.device) * eps
        _, x = predictor_obj.update_fn(x, vec_eps, y=y)
        return x

    def drift_fn(model, x, t, y=None):
        """Get the drift function of the reverse-time SDE."""
        score_fn = get_score_fn(sde, model, continuous=True)
        rsde = sde.reverse(score_fn, probability_flow=True)
        return rsde.sde(x, t, y=y)[0]
    ...
    def ode_sampler(model, z=None):
        ...
        with torch.no_grad():
            ...
            def ode_func(t, x):
                ...
                drift = drift_fn(model, x, vec_t, y=y)
            ...
            # adding noise
            if denoise:
                x = denoise_update_fn(model, x, y=y)
